<?php
/*
 * bootstrap.php — boots the congruency framework (submodule under vendor/)
 ...
 *
 * Env knobs:
 *   CONGRUENCEY_PATH      path to the code under test (default: vendor/congruency)
...
$congruency = getenv('CONGRUENCEY_PATH');
if ($congruency === false || $congruency === '') {
    $congruency = BUGREPO_ROOT . '/vendor/congruency';
}
$congruency = rtrim($congruency, '/');
if (!is_file($congruency . '/www/Constants.php')) {
    fwrite(STDERR, "Cannot find congruency at: $congruency\n"
...
require $congruency . '/www/Constants.php';
...
$GLOBALS['__congruency_loader'] = getClassLoader();
spl_autoload_register(function ($class) {
    $GLOBALS['__congruency_loader']->loadClassByName($class);
});
...
function bug_report(string $expected, callable $trigger): void {
    ...
    try {
        ...
        echo "  at " . str_replace(BUGREPO_ROOT . '/vendor/congruency/', '', $e->getFile())
           . ":" . $e->getLine() . "\n";
        echo "REPRODUCED\n";
    }
}
