...
| **Bug catalog** | all 15 catalogued bugs still reproduce against the pinned submodule | `../congruency-bugs/run` |
...
- `congruency-harness/` — boots the app (static PHP 8.3, shim, seeded SQLite). Provides `php/php` and the bootstrap this suite loads.
- `congruency/` — the code under test (`main`, with the fixes).
- `congruency-bugs/` — the bug catalog (pinned submodule at the original commit).
