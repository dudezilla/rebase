...
A side repository that catalogs bugs in [`congruencey`](https://github.com/dudezilla/congruencey)
...
The code under test is pinned here as a git submodule at [`vendor/congruencey`](vendor/congruencey),
so every reproduction runs the genuine 2006 classes, not a paraphrase.

## Quick start

```bash
git clone <this repo> && cd congruencey-bugs
...
vendor/congruencey submodule: the code under test (pinned)
```
